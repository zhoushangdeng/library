...
const logV = require('../Log').getLogger("Router:borrow")
...
const selectBorrow = async ctx => {
    ...
    logV.trace("select borrow", { start, end, userId })
    const sql = userId
...
const createBorrow = async ctx => {
    ...
    logV.trace("create borrow", { bookId, studentCode, borrowDate, returnDate, state })
    ...
    logV.trace("create borrow success", ctx.body)
}

const updateBorrow = async ctx => {
    const { id, bookId, studentCode, borrowDate, returnDate, state } = ctx.request.body;
    logV.trace("update borrow", { bookId, studentCode, borrowDate, returnDate })
    ...
    ctx.body = { code: 200, result }
    logV.trace("update borrow success", ctx.body)
}

const deleteBorrow = async ctx => {
    const { id } = ctx.request.query;
    logV.trace("delete Borrow", { id })
    const result = await query(`delete from borrow where id=${id}`)
    ctx.body = { code: 200, result }
    logV.trace("delete Borrow success", ctx.body)
}
